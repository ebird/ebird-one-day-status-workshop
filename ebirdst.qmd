...
ebirdst_download_status("piecur1", pattern = "abundance_median_27km", 
                        force = TRUE)
```
:::
...
#| eval: false
path <- ebirdst_download_status(species = "Scarlet Myzomela")
path
```
...
#| echo: false
path <- get_species_path("Scarlet Myzomela")
path
```
...
For a species whose data have already been downloaded, you can use `get_species_path("Scarlet Myzomela")` to identify the path to the data.

::: callout-tip
## Tip

<!-- TODO update #sec-intro-data anchor -->

To avoid any issues resulting from all participants downloading data over a slow internet connection at the same time, for the remainder of this lesson we'll use data included in the [data package for this workshop](index.qmd#sec-intro-data). These data packages should be in the `data/ebirdst-data/` sub-directory of your RStudio project. To find the path to the data for a particular species using a non-standard download location, use

<!-- TODO come back to this one I finalized the species packages -->
<!-- TODO get_species_path for a local path is not working -->

```{r}
#| label: ebirdst-load-path
#| eval: false
path <- get_species_path("scamyz1", path = "data/ebirdst-data/2022/")
path
```
...
# TODO put this path back , path = "data/ebirdst-data/"
...
ggplot(trajectories, aes(x = week, y = prop_pop, color = species)) +
  ...
  labs(x = "Week", 
       y = "% of population",
       title = "Weekly % of population trajectory",
       color = NULL) +
  theme_bw() +
...
for (i in seq_len(nrow(species_list))) {
  ...
  percent_population[[this_sp$species_code]] <- pop
}
...
The high importance sites we identified are shown in green, while the existing protected area network is overlaid in grey. It appears the existing protected area network is doing a better job in northern Queensland than in southeast Queensland for these species. This might not  besurprising since the location of existing protected areas was not chosen specifically to protect these species. Let's quantify what proportion of the population these two regions capture.
